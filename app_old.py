...
def main():
    ...
    # Check for dashboard file (try both possible names)
    dashboard_path = None
    possible_names = ["summary_dashboard.html", "main_dashboard.html"]
    
    for name in possible_names:
        path = script_dir / "outputs" / "figures" / name
        if path.exists():
            dashboard_path = path
            break
    ...
    if dashboard_path and dashboard_path.exists():
        ...
        # List available HTML files in the figures directory
        figures_dir = script_dir / "outputs" / "figures"
        if figures_dir.exists():
            html_files = list(figures_dir.glob("*.html"))
            if html_files:
                print(f"   ℹ Available visualizations:")
                for html_file in sorted(html_files):
                    print(f"      • {html_file.name}")
                print(f"\n   💡 Open any of these files in your browser to view the visualizations")
            else:
                print(f"   ✗ No HTML visualization files found in {figures_dir}")
        else:
