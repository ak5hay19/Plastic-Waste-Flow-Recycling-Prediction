...
class Visualizer:
    ...
    def plot_forecast_comparison(self, historical_df, forecast_dict, column='Production_Tonnes', save=True):
        ...
        # Ensure we have the right index format
        if column in historical_df.columns:
            hist_y = historical_df[column].values
            # Get x-axis values from index
            if isinstance(historical_df.index, pd.DatetimeIndex):
                hist_x = historical_df.index.year
            elif hasattr(historical_df.index, 'tolist'):
                hist_x = historical_df.index.tolist()
            elif 'Year' in historical_df.columns:
                hist_x = historical_df['Year'].values
            else:
                hist_x = list(historical_df.index)
        else:
            # If column not found, try to use the first numeric column
            numeric_cols = historical_df.select_dtypes(include=[np.number]).columns
            if len(numeric_cols) > 0:
                hist_y = historical_df[numeric_cols[0]].values
                if isinstance(historical_df.index, pd.DatetimeIndex):
                    hist_x = historical_df.index.year
                elif hasattr(historical_df.index, 'tolist'):
                    hist_x = historical_df.index.tolist()
        ...
        for i, (model_name, forecast_df) in enumerate(forecast_dict.items()):
            ...
            # Filter out NaN values
            forecast_clean = forecast_df.dropna(subset=['Forecast'])
            
            if len(forecast_clean) == 0:
                print(f"Warning: No valid forecasts for {model_name}, skipping...")
                continue
            
            # Ensure index is properly formatted
            forecast_x = forecast_clean.index
            
            # Convert index to numeric if it's datetime or other types
            if isinstance(forecast_x, pd.DatetimeIndex):
                forecast_x = forecast_x.year.tolist()
            elif hasattr(forecast_x, 'tolist'):
                forecast_x = forecast_x.tolist()
            else:
                forecast_x = list(forecast_x)
            
            forecast_y = forecast_clean['Forecast'].values
