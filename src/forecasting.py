...
class Forecaster:
    ...
    def forecast_arima(self, timeseries, periods=10, order=(1, 1, 1), column='Production_Tonnes'):
        ...
        # Make predictions
        try:
            # Use get_forecast for better error handling
            forecast_result = fitted_model.get_forecast(steps=periods)
            forecast = forecast_result.predicted_mean
            forecast_conf_int = forecast_result.conf_int()
        except Exception as e:
            print(f"Warning: get_forecast failed, trying forecast method: {e}")
            try:
                forecast = fitted_model.forecast(steps=periods)
                forecast_conf_int = None
            except Exception as e2:
                print(f"Warning: forecast method also failed, trying predict: {e2}")
                # Try using predict instead
                last_idx = len(ts) - 1
                forecast = fitted_model.predict(start=last_idx + 1, end=last_idx + periods, typ='levels')
                forecast_conf_int = None
        ...
        if np.any(np.isnan(forecast_values)):
            print(f"Warning: Forecast contains NaN values. Attempting to fill with extrapolation...")
            # Try to fill NaN with linear extrapolation based on last few values
            last_values = ts[column].values[-5:]
            if len(last_values) > 1 and not np.any(np.isnan(last_values)):
                # Calculate trend
                trend = (last_values[-1] - last_values[0]) / len(last_values) if len(last_values) > 1 else 0
                last_value = last_values[-1]
                # Fill NaN values
                for i in range(len(forecast_values)):
                    if np.isnan(forecast_values[i]):
                        forecast_values[i] = last_value + trend * (i + 1)
                        last_value = forecast_values[i]
                forecast = pd.Series(forecast_values, index=forecast.index if hasattr(forecast, 'index') else None)
        
        # Create forecast dataframe with proper index
        last_year = timeseries.index[-1]
        if isinstance(last_year, (int, np.integer)):
            # If index is integer years, create integer range
            forecast_index = pd.Index(range(int(last_year) + 1, int(last_year) + periods + 1), name='Year')
        elif isinstance(last_year, pd.Timestamp):
            # If index is datetime, create date range
            forecast_index = pd.date_range(start=last_year + pd.DateOffset(years=1), periods=periods, freq='YS')
        else:
            # Try to convert to year if possible
            try:
                last_year_int = int(last_year)
                forecast_index = pd.Index(range(last_year_int + 1, last_year_int + periods + 1), name='Year')
            except:
                # Fallback: use numeric index
                forecast_index = pd.RangeIndex(start=len(timeseries), stop=len(timeseries) + periods)
    ...
    def forecast_prophet(self, timeseries, periods=10, yearly_seasonality=True):
        ...
        # Determine the value column (not 'Year', 'index', 'Entity', 'Code')
        exclude_cols = ['Year', 'index', 'Entity', 'Code']
        value_col = [col for col in df.columns if col not in exclude_cols][0]
        
        # If Entity column exists and has multiple values, filter to keep only one
        # (Prophet needs a single time series)
        if 'Entity' in df.columns:
            unique_entities = df['Entity'].unique()
            if len(unique_entities) > 1:
                # If multiple entities, use the first one (or could aggregate)
                print(f"Warning: Multiple entities found. Using first entity: {unique_entities[0]}")
                df = df[df['Entity'] == unique_entities[0]].copy()
            # If only 'World', that's fine - keep it
        ...
        if 'Year' not in df.columns:
            if df.index.name == 'Year':
                ...
                raise ValueError("Could not find 'Year' column or index")
        
        # Create Prophet dataframe - ensure Year is numeric
        try:
            prophet_df = pd.DataFrame({
                'ds': pd.to_datetime(df['Year'].astype(int).astype(str), format='%Y'),
                'y': pd.to_numeric(df[value_col], errors='coerce')
            })
        except Exception as e:
            print(f"Error creating Prophet dataframe: {e}")
            # Try alternative approach
            prophet_df = pd.DataFrame({
                'ds': pd.to_datetime(df['Year'], errors='coerce'),
                'y': pd.to_numeric(df[value_col], errors='coerce')
            })
        ...
        if len(prophet_df) == 0:
            raise ValueError("No valid data remaining after cleaning for Prophet forecast")
